# pages/1_📥_Input_&_Hasil.py — Ketua by beban(aktif), Anggota STRICT same SK row (robust + header standardizer), PP/JS rotate
...
def _standardize_cols(df: pd.DataFrame) -> pd.DataFrame:
    """Rename likely variant headers to canonical names and clean strings."""
    if df is None or df.empty: return pd.DataFrame()
    ren = {}
    for c in list(df.columns):
        raw = str(c).replace("\ufeff","").strip()  # drop BOM on first header
        ...
        elif k in {"pp1","panitera pengganti 1"}: new = "pp1"
        elif k in {"pp2","panitera pengganti 2"}: new = "pp2"
        ...
        else: new = raw  # keep original
        ren[c] = new
    out = df.rename(columns=ren).copy()
    # Clean all string cells (trim, unify spaces)
...
def _best_sk_row_for_ketua(sk: pd.DataFrame, ketua: str) -> pd.Series | None:
    """Robust matcher: prefer aktif rows; then highest token overlap; then lowest majelis rank."""
    if sk is None or sk.empty or not ketua: return None
    df = _standardize_cols(sk)
    if "ketua" not in df.columns:
        return None
    ...
    # candidates: token-overlap OR exact key
    ...
    if "aktif" in cand.columns:
        cand["__aktif"] = cand["aktif"].apply(_is_active_value)
    else:
        cand["__aktif"] = True
    if "majelis" in cand.columns:
        cand["__rank"] = cand["majelis"].astype(str).map(_majelis_rank)
    else:
        cand["__rank"] = 10**9
...
def _pick_ketua_by_beban(hakim_df: pd.DataFrame, rekap_df: pd.DataFrame) -> tuple[str, pd.Series | None]:
    """Pilih ketua berdasarkan beban (total baris rekap) di antara hakim yang aktif."""
...
# ==== Debug kandidat baris SK ====
with st.expander("🔍 Debug Pencocokan SK Ketua"):
    if isinstance(sk_df, pd.DataFrame) and not sk_df.empty and hakim:
        df_dbg = _standardize_cols(sk_df.copy())
        df_dbg["__ketua_key"] = df_dbg.get("ketua","").astype(str).map(_name_key) if "ketua" in df_dbg.columns else ""
        target = _tokset(hakim)
        if "ketua" in df_dbg.columns:
            df_dbg["__overlap"] = df_dbg["ketua"].astype(str).apply(lambda s: len(_tokset(s) & target))
            if "aktif" in df_dbg.columns:
                df_dbg["__aktif"] = df_dbg["aktif"].apply(_is_active_value)
            else:
                df_dbg["__aktif"] = True
            if "majelis" in df_dbg.columns:
                df_dbg["__rank"] = df_dbg["majelis"].astype(str).map(_majelis_rank)
            else:
                df_dbg["__rank"] = 10**9
            show = df_dbg.sort_values(["__aktif","__overlap","__rank"], ascending=[False,False,True])
            st.dataframe(show[[c for c in show.columns if not c.startswith("__")]+["__overlap","__aktif","__rank"]].head(50), use_container_width=True)
        else:
            st.warning("Kolom 'ketua' tidak ada pada SK. Coba periksa header file SK.")
    else:
        st.info("Belum ada SK atau ketua kosong.")
