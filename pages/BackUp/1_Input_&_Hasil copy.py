...
st.write("DEBUG call auto-pick OK sampai sini")
...
with tab1:
    ...
    with right:
        ...
        # --- dropdown ketua + label cuti (tetap sama seperti sebelumnya) ---
        cuti_df = _load_cuti_df(_cuti_mtime())
        ...
        visible_names, hidden_reasons, cuti_names, label_map = [], {}, [], {}
        if isinstance(hakim_df, pd.DataFrame) and (not hakim_df.empty) and ("nama" in hakim_df.columns):
            ...
            for _, r in df_sorted.sort_values(["__nama_clean"], kind="stable").iterrows():
                nm = r["__nama_clean"]; tgl = r["__tgl_rencana"]
                if tgl is None:
                ...
                if cuti_today or cuti_on_plan:
                    if show_cutis:
                        ...
                        jbtn = _get_jabatan_for(nm); extra = f" • {jbtn}" if jbtn and jbtn.strip() else ""
                        cuti_label = " & ".join((["HARI INI"] if cuti_today else []) + ([f"{format_tanggal_id(pd.to_datetime(tgl))}"] if cuti_on_plan else []))
        ...
        if not visible_names:
            if isinstance(sk_df, pd.DataFrame) and (not sk_df.empty) and ("ketua" in sk_df.columns):
                fallback = (sk_df["ketua"].astype(str).map(str.strip)
                            .replace("", pd.NA).dropna())
                fallback = fallback[~fallback.map(_is_header_like)].drop_duplicates().sort_values().tolist()
        ...
        hakim_manual = st.selectbox(
            ...
            [""] + visible_names,
            format_func=lambda x: ("" if x == "" else label_map.get(x, x)),
            key=K("t1", f"hakim_manual_{fs}")
        )
        if isinstance(hakim_manual, str) and hakim_manual.strip() == "":
            hakim_manual = ""
    ...
    # --- Tentukan Ketua & SK (live) ---
    if str(hakim_manual or "").strip():
        ketua = str(hakim_manual).strip()
        sk_row = _best_sk_row_for_ketua(sk_df, ketua)
        if sk_row is None:
            st.warning("Ketua manual tidak ditemukan di SK. Anggota/PP/JS akan dikosongkan.")
    else:
        ketua, sk_row = _pick_ketua_by_beban(hakim_df, rekap_df, tgl_register_input, jenis, klas_final, libur_df)

    if not ketua:
        st.error("Auto-pick ketua menghasilkan kosong padahal kandidat ada. Cek data SK/Hakim/Hari Sidang.")
    else:
        st.caption(f"Auto-pick ketua: **{ketua}**")
    ...
    # Hitung Tgl Sidang (live)
    base = tgl_register_input if isinstance(tgl_register_input, (datetime, date)) else date.today()
    hari_sidang_num = 0
    if (isinstance(hakim_df, pd.DataFrame) and not hakim_df.empty and hakim and "nama" in hakim_df.columns
    ...
    libur_set = _libur_set_from_df(libur_df)
    tgl_sidang_auto = _compute_tgl_sidang(base.date() if isinstance(base, datetime) else base,
                                          jenis, hari_sidang_num, libur_set, klasifikasi=klas_final)

    # Override tanggal (live)
    with st.expander("🗓️ Override Tanggal Sidang (opsional)", expanded=False):
        use_override = st.checkbox("Gunakan tanggal sidang manual", value=False, key=K("t1", f"use_override_{fs}"))
        override_raw = st.date_input("Tanggal Sidang (manual)", value=tgl_sidang_auto, key=K("t1", f"override_raw_{fs}"))
        mode = st.radio("Mode tanggal manual",
    ...
    with resL:
        st.write("**Hakim (Ketua):**", hakim if hakim else "-")
    ...
    simpan = st.button("💾 Simpan ke Rekap (CSV)", key=K("t1","save_btn"), disabled=not (bool(hakim) and str(nomor).strip() != ""))
    if simpan:
